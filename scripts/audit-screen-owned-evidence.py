...
target=ROOT/'evidence/screen-owned-validation.json';assert not target.exists()
binary=ROOT/'target/screen-owned-candidate/release/rboxc';repro=ROOT/'target/screen-owned-repro/release/rboxc'
...
assert new['native_command_entries']==[]
changed=[]
...
reports={n:json.loads((ROOT/'evidence'/(n+'.json')).read_text()) for n in [
 'screen-owned-original-detach','screen-owned-recovery','screen-owned-contract',
 'screen-owned-focused','screen-owned-smoke','screen-owned-full-dispatch',
...
def memory(entry,path_key,hash_key):
 ...
 assert all(entry[k]==v for k,v in parsed.items())
 return parsed

def clean(m):
 return m['complete_exec_log'] and m['errors']==m['non_inherited_descriptors']==0 and not any(m['heap_bytes'].get(k,0) for k in ['definitely lost','indirectly lost','possibly lost'])

for name,expected in [('screen-owned-focused',5),('screen-owned-smoke',428),('screen-owned-full-dispatch',11)]:
 d=reports[name];assert d['passed']==d['total']==expected and d['binary_sha256']==fingerprint(binary)
for name in ['screen-owned-original-detach','screen-owned-recovery']:
...
contract=reports['screen-owned-contract'];assert contract['passed']==contract['total']==19 and contract['processes']==26
...
helper_original=json.loads((ROOT/'build/screen-before-key-cleanup/screen-link.json').read_text())
for path,h in new['helper_inputs'].items():
 if path.endswith(('-winmsgbuf.o','-winmsgcond.o')):assert helper_original['helper_inputs'][path]==h
...
report={'scope':'Screen daemon exit closes only its successfully reopened standard streams and owned server '
 ...
 'ownership_contracts':19,'clean_contract_processes':26,'focused_passed':5,
 ...
 'prior_validation':{'path':'evidence/screen-daemon-validation.json','sha256':fingerprint(ROOT/'evidence/screen-daemon-validation.json')},
 ...
 'native_incomplete_process_logs':[p for p in processes if p['implementation']=='gnu-valgrind' and not p['complete_exec_log']],
...
print('Screen daemon audit passed: 12 terminal process logs and 26 contract process logs clean')
