...
rboxc_discard_trap_borrow (void *state)
{
  ...
  /* This callback only runs when the interpreter abandons the handler.
     An inherited special trap can still occupy its slot after GNU's
     free_trap_strings. Detach it before releasing the last live borrow. */
  if (borrow->string)
    {
      if (trap_list[borrow->sig] == borrow->string)
        {
          trap_list[borrow->sig] = 0;
          sigmodes[borrow->sig] &= ~(SIG_TRAPPED|SIG_INPROGRESS|SIG_CHANGED);
        }
  ...
  rboxc_unlink_trap_borrow (state);
    ...
    body = body.replace(old, '''      else
        {
          /* As in run_pending_traps, finish the interrupted parse frame
             before discarding this handler's state registrations. */
  ...
  add_unwind_protect_owned (xfree, xfree, old_trapsig);
#if defined (ARRAY_VARS)
  add_unwind_protect_owned (rboxc_discard_trap_status,
                            rboxc_discard_trap_status, ps);
#endif''')
    old = '\t      save_parser_state (&pstate);'
    assert body.count(old) == 1
...
'''+old)
    ...
    (root/'evidence/bash-trap-restart-cleanup.json').write_text(json.dumps({
        'scope':'Register discard-only owners for trap parser state, PIPESTATUS and BASH_TRAPSIG snapshots, including the outer pending-signal dispatcher. Track active handler borrows and invalidate them at every actual string release; restart releases live borrows, detaching a still-inherited slot first. Normal returns retain GNU restoration and unlink ownership records. A pending trap returning out of a function disposes its otherwise abandoned status copy without restoring it.',
