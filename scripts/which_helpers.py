"""Isolate GNU Which helpers and their references to Rust-owned definitions."""
...
def native_inputs(root):
    return [root/'build/gnu-which'/name for name in ('getopt.o', 'getopt1.o', 'bash.o', 'tilde.o')]
...
def symbol_map(root):
    symbols = defined_symbols([*native_inputs(root), root/'build/gnu-which/which.o'])
    assert 'main' in symbols
    symbols.remove('main')
    assert not any(s.startswith('single_binary_main_') for s in symbols)
    return {s: 'rboxc_which_'+s for s in sorted(symbols)}
...
def prepare_archive(root, mapping):
    ...
    archive = root/'build/helpers/libwhich-rboxc.a'
