#!/usr/bin/env python3
"""Link the two translated Cpio entries with independent native helpers."""
# SPDX-License-Identifier: GPL-3.0-or-later
import json
from pathlib import Path
import re
import subprocess
from cpio_helpers import COMMANDS, fingerprint, native_inputs, prepare_archives, symbol_map
...
for command in COMMANDS:
    ...
    entry = json.loads((ROOT/f'evidence/cpio-{command}-translation.json').read_text())
...
makefile = ROOT/'build/cpio-print-link.mk'
makefile.write_text(".PHONY: rboxc-cpio-link\nrboxc-cpio-link:\n\t@printf '%s\\n' '$(LDADD) $(LIBS)'\n")
recorded = subprocess.check_output(['make', '--no-print-directory', '-s', '-f', 'Makefile',
    '-f', makefile, 'rboxc-cpio-link'], cwd=ROOT/'build/gnu-cpio/src', text=True).split()
assert recorded == ['../lib/libpax.a', '../gnu/libgnu.a'], 'unreviewed native link inputs'
...
report_path = ROOT/'evidence/cpio-link.json'
...
report = {'provider': 'cpio', 'entries': [translations[n]['entry'] for n in COMMANDS],
          'native_command_entries': [], 'scope': 'Two Rust entries with command-private Pax/Gnulib state. Both original C main objects are excluded.',
...
print('Assembled two GNU Cpio Rust entries without C main')
