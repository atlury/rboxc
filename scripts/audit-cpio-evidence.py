#!/usr/bin/env python3
"""Verify current Cpio comparisons and reparse every candidate process log."""
...
for row in original['results']:
    assert row['pass'] and row['assertions_pass']
    for key,outcome in row['outcomes'].items():
        ...
        for log in outcome['memory']:
            ...
            assert len(commands)==1 and commands[0].split()[0]=='cpio', 'unclassified child process'
