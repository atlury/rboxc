#!/usr/bin/env python3
"""Translate the pinned GNU BC entry and namespace its helper imports."""
...
import re
import subprocess
from bc_helpers import COMMANDS, defined_symbols, fingerprint, symbol_map
...
source = Path(os.environ.get('GNU_BC_SOURCE', pin['source']))/pin['entry_source'][name]
expected = pin['entry_sha256'][name]
assert fingerprint(source) == expected
records = [json.loads(p.read_text()) for p in (ROOT/'build/bc-cc-records').glob('*.json')]
...
def renamed_export(match):
    symbol = match['name']
    if symbol.startswith('rboxc_bc_'):
        return match[0]
...
report = {'provider': 'bc', 'version': pin['version'], 'command': name,
