#!/usr/bin/env python3
"""Prepare the original Gawk English UTF-8 test locale from pinned glibc data."""
...
name='en_US.UTF-8';stage=ROOT/'build/gawk-english-locale-final-stage';stage.mkdir(exist_ok=True)
runtime=Path('/usr/lib/locale/rboxc-gawk-english');runtime.mkdir(exist_ok=True)
report_path=ROOT/'evidence/gawk-english-locales.json';assert not report_path.exists()
...
command=['/usr/bin/localedef','--no-archive','-i',str(source/'locales/en_US'),'-f',str(source/'charmaps/UTF-8'),str(stage/name)]
env={**os.environ,'I18NPATH':str(source)}
log=ROOT/'evidence/raw/gawk-english-locale-final-build.log'
...
(runtime/'en_US.utf8').symlink_to(name)
...
report_path.write_text(json.dumps({'scope':'Separate English UTF-8 collection for the unchanged original POSIX comparison and string-length Make recipes. Pinned glibc locale and UTF-8 source data; system locale archive unchanged. Explicit LOCPATH is supplied only to this test.','preparation_driver':'scripts/prepare-gawk-english-locales.py','driver_sha256':digest(Path(__file__)),'runtime_path':str(runtime),'name':name,'alias':'en_US.utf8','inputs':inputs,'files':files,'build_command':command,'build_log':str(log.relative_to(ROOT)),'build_log_sha256':digest(log),'probe':{'status':probe.returncode,'stdout':probe.stdout.decode(),'stderr':probe.stderr.decode()}},indent=2)+'\n')
print('Prepared and verified private English UTF-8 locale')
