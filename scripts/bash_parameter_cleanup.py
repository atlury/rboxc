...
'''+anchor+'''
  ...
  discard_unwind_frame ("rboxc-brace-pattern-value");''')
    for function in ('expand_string_for_pat','expand_string_for_rhs'):
        start=text.index('\n'+function+' (')
        end=text.index('\n}\n',start)+3
        old=text[start:end]
        new=old.replace('  WORD_DESC td;','  WORD_DESC *td;')
        new=new.replace('  td.flags =','  td = alloc_word_desc ();\n  td->flags =',1)
        new=new.replace('td.flags','td->flags').replace('td.word','td->word').replace('(&td,','(td,')
        new=new.replace('  td->word = savestring (string);','''  td->word = savestring (string);
  ...
  add_unwind_protect_heap (rboxc_dispose_expansion_word, rboxc_dispose_expansion_word, td);''')
        new=new.replace('  free (td->word);','''  discard_unwind_frame ("rboxc-brace-expansion-word");
  dispose_word (td);''')
        assert new!=old and 'WORD_DESC td;' not in new and '&td' not in new
        replace(old,new)
    ...
    (root/'evidence/bash-parameter-cleanup.json').write_text(json.dumps({
        'scope':'Brace length names, recursive nameref strings and pattern caller allocations retain existing xfree unwind owners during nested expansion. Pattern and RHS expansion descriptors live on the heap with explicit heap-only cleanup. Whole arrays and positional parameters remain borrowed. Normal calls discard temporary registrations and retain original release semantics.',
