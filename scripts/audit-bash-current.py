...
for row in inventory['inputs']:
    ...
    assert row['state'] in ('reviewed-original-pass','reviewed-original-assertions-memory-open')
    ...
    for case in row.get('script_cases',[{}]):
        ...
        assert all(o['expected_output_matches'] and not o.get('timed_out',False) for o in result['outcomes'].values())
        ...
        results.append({'selection':selection,'pass':result['pass'],'binary_sha256':report['binary_sha256'],'evidence':row['evidence'],'audit':row['audit']})
...
target.write_text(json.dumps({
    ...
    'memory_open_scripts':[r['selection'] for r in results if not r['pass']],
