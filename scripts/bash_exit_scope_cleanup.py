...
'''+anchor)
            replace('      add_unwind_protect (uw_restore_pipestatus_array, psa);',
...
'''+anchor)
        ...
        if name=='evalstring.c':
            archive=stage/'libbuiltins.a'
            shutil.copyfile(adapted[archive.name],archive)
            subprocess.run(['ar','r',str(archive),str(obj)],check=True)
            subprocess.run(['ranlib',str(archive)],check=True)
            adapted[archive.name]=archive
        else:
