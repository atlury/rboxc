#!/usr/bin/env python3
"""Bind the Cpio candidate to complete current regression evidence."""
...
candidate = ROOT/'target/cpio-directory-cleanup-candidate/release/rboxc'
binary_hash = digest(candidate)
rebuilt = ROOT/'target/cpio-repro-check/release/rboxc'
assert digest(rebuilt) == binary_hash, 'fresh source rebuild differs from tested executable'
assert len(subprocess.check_output([candidate,'--list'],text=True).splitlines()) == 133
mapping = {
    ...
    'cpio-behavior':'cpio-combined-cpio-behavior',
    'cpio-memory-audit':'cpio-cleanup-memory-audit',
}
providers = ('hello','time','which','diffutils','grep','gzip','sed','bc','ed','findutils','tar','sharutils')
expected_focused = dict(zip(providers,(17,23,46,56,68,56,56,44,44,83,57,72)))
for provider in providers:
    mapping[provider+'-behavior'] = 'cpio-combined-'+provider+'-behavior'
    if provider != 'which':
        mapping[provider+'-original-reuse'] = 'cpio-'+provider+'-original-reuse'
reports = {}
for canonical,name in mapping.items():
    ...
    if canonical.endswith('-behavior'):
        ...
        expected = expected_focused.get(provider, {'cpio':52}.get(provider))
    ...
    if canonical == 'cpio-original':
        assert data['complete'] and data['total'] == 10
    if canonical == 'cpio-memory-audit':
        assert data['total'] == 95 and data['original_processes'] == 43
        assert data['focused_report']['sha256'] == digest(ROOT/'evidence/cpio-combined-cpio-behavior.json')
        assert data['original_report']['sha256'] == digest(ROOT/'evidence/cpio-cleanup-original.json')
...
sources = {}
for provider in (*providers,'cpio'):
...
proof = {'scope':'Activate cpio and mt Rust entries after ten reviewed GNU original selections, 52 focused checks, and fresh combined regressions. Cpio has 43 clean original processes, plus 52 focused processes. Broader originals and real tape-device operations remain unvalidated. Earlier provider originals retain actual executable hashes and open findings under verified unchanged inputs and fresh focused checks.',
         ...
         'reproducibility':{'rebuilt_binary':str(rebuilt.relative_to(ROOT)),'sha256':digest(rebuilt),
                            'build_log':'evidence/raw/cpio-repro-check-build.log',
                            'build_log_sha256':digest(ROOT/'evidence/raw/cpio-repro-check-build.log')},
         'needed_libraries':needed(candidate),'reports':reports,'source_evidence':sources}
path = ROOT/'evidence/cpio-candidate-proof.json'
assert not path.exists(), 'preserve previous proof'
path.write_text(json.dumps(proof,indent=2)+'\n')
print('Proved 133-command candidate with',len(reports),'current validation reports')
