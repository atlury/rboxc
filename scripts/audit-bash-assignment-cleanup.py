#!/usr/bin/env python3
"""Audit assignment ownership, original regression profiles and shared candidate checks."""
...
target=ROOT/'evidence/bash-assignment-cleanup-validation.json';assert not target.exists()
binary=ROOT/'target/bash-assignment-cleanup-candidate/release/rboxc';digest=sha(binary)
assert sha(ROOT/'target/bash-assignment-cleanup-repro/release/rboxc')==digest
...
def pin(path,expected=None):
 ...
 return h
p=ROOT/'evidence/bash-assignment-cleanup.json';profile=json.loads(p.read_text());pin('scripts/bash_assignment_cleanup.py',profile['driver_sha256'])
...
a=json.loads((ROOT/'evidence/bash-before-assignment-cleanup-link.json').read_text());b=json.loads((ROOT/'evidence/bash-link.json').read_text())
assert a['original_inputs']==b['original_inputs']
assert a['helper_inputs'].keys()==b['helper_inputs'].keys()
changed=[p for p in a['helper_inputs'] if a['helper_inputs'][p]!=b['helper_inputs'][p]];assert changed==['build/helpers/bash-010-expr.o','build/helpers/bash-013-subst.o','build/helpers/bash-026-arrayfunc.o']
for p,h in b['helper_inputs'].items():pin(p,h)
original_processes=[]
for report_name,audit_name,count in [('bash-assignment-cleanup-original','bash-assignment-original-validation',1),('bash-assignment-regression-original','bash-assignment-regression-validation',5)]:
...
for name,total in [('contract',32),('behavior',44),('adapters',69),('smoke',428),('dispatch',11)]:
 p=ROOT/f'evidence/bash-assignment-cleanup-{name}.json';r=json.loads(p.read_text());assert r.get('complete',True) and r['binary_sha256']==digest
 ...
 reports[str(p.relative_to(ROOT))]=pin(p)
for p in ['scripts/entry_provider_helpers.py','tests/bash-original.py','tests/bash-assignment-contract.py','evidence/raw/bash-assignment-cleanup-original-inventory.json','evidence/bash-before-assignment-cleanup-link.json','evidence/bash-link.json']:pin(p)
target.write_text(json.dumps({'scope':'Three isolated Bash helpers retain ownership of arithmetic lookahead tokens, assignment values and expanded array lists through ordinary diagnostics. The unchanged exp-tests original and five related originals pass strictly. Thirty-two assignment contracts and shared checks pass on the byte-identical candidate. Earlier results retain their actual tested hashes; full Bash/GNU acceptance remains incomplete.',
...
print('PASS: assignment ownership; six strict originals;',len(original_processes),'clean processes')
