#!/usr/bin/env python3
"""Link three Findutils Rust entries with isolated provider helpers and state."""
# SPDX-License-Identifier: GPL-3.0-or-later
import json
from pathlib import Path
import re
import subprocess
from findutils_helpers import COMMANDS, fingerprint, native_inputs, prepare_archives, symbol_map
...
for name in COMMANDS:
    ...
    entry = json.loads((ROOT/f'evidence/findutils-{name}-translation.json').read_text())
...
makefile = ROOT/'build/findutils-print-link.mk'
makefile.write_text(".PHONY: rboxc-findutils-link\nrboxc-findutils-link:\n\t@printf '%s\\n' '$(LDADD) $(LIBS)'\n")
flags = []
recorded = {}
for name in COMMANDS:
    inputs = subprocess.check_output(['make', '--no-print-directory', '-s', '-f', 'Makefile',
        '-f', makefile, 'rboxc-findutils-link'], cwd=ROOT/'build/gnu-findutils'/name, text=True).split()
    archives = ['libfindtools.a'] if name == 'find' else []
    archives += ['../lib/libfind.a','../gl/lib/libgnulib.a']
    assert inputs[:len(archives)] == archives
    extra_flags = inputs[len(archives):]
    assert set(extra_flags) <= {'-lm'}, 'unreviewed link input'
...
previous_path = ROOT/'evidence/findutils-link.json'
...
report = {'provider': 'findutils', 'entries': [translations[n]['entry'] for n in COMMANDS],
          'native_command_entries': [], 'scope': 'Three Rust entries with separate native helper/state namespaces. GNU findtools, find, and gnulib helper archives retain the original command linkage; native entry objects are excluded.',
...
(ROOT/'evidence/findutils-link.json').write_text(json.dumps(report, indent=2)+'\n')
print('Assembled three GNU Findutils entries with', sum(len(m) for m in mappings.values()), 'private symbols and no C main')
