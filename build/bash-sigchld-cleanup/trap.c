	  ...
	  if (sig == SIGINT)
	      ...
	      else
		{
	      ...
	      subst_assign_varlist = save_subst_varlist;
              discard_unwind_frame ("rboxc-pending-trap-parser");
		  ...
		  if (function_code)
		    {
		      ...
		      trap_return_context = old_context;
                      discard_unwind_frame ("rboxc-pending-trap-state");
#if defined (ARRAY_VARS)
                      array_dispose (ps);
#endif
...
_run_trap_internal (int sig, char *tag)
{
  ...
  if ((sigmodes[sig] & SIG_TRAPPED) && ((sigmodes[sig] & SIG_IGNORED) == 0) &&
	...
	restore_pipeline (1);
      ...
      rboxc_unlink_trap_borrow (&rboxc_borrow);
...
#endif
	    rboxc_release_trap_string (old_trap);
	  sigmodes[sig] &= ~SIG_CHANGED;
