...
add_one_character:
  ...
  *sindex = i;
  ...
  discard_unwind_frame ("rboxc-extract-quoted");
  return (temp);
}
	  ...
	  if (MB_INVALIDCH (mblength))
	      ...
	      if (wcharlist == 0)
		{
		  ...
		  wcharlist = (wchar_t *)xmalloc (sizeof (wchar_t) * (len + 1));
                  /* A failed sizing conversion above deliberately leaves an
...
do_compound_assignment (const char *name, char *value, int flags)
{
  ...
  if (mklocal && variable_context)
	...
	v = make_local_assoc_variable (newname, 0);
      ...
      discard_unwind_frame ("rboxc-compound-list");
      if (list)
	dispose_words (list);
...
do_assignment_internal (const WORD_DESC *word, int expand)
{
  ...
  begin_unwind_frame ("rboxc-assignment-value");
...
do \
{ \
  discard_unwind_frame ("rboxc-assignment-value"); \
  FREE (value); \
...
expand_string_internal (const char *string, int quoted)
{
  ...
  /* A heap descriptor survives a nonlocal expansion error. GNU's
     call_expand_word_internal clears its word when that word was freed. */
  td = alloc_word_desc ();
  td->word = savestring (string);
  begin_unwind_frame ("rboxc-string-expansion");
  add_unwind_protect_heap (rboxc_dispose_expansion_word,
                            rboxc_dispose_expansion_word, td);

  tresult = call_expand_word_internal (td, quoted, 0, (int *)NULL, (int *)NULL);

  discard_unwind_frame ("rboxc-string-expansion");
  dispose_word (td);
  return (tresult);
}
...
expand_string_assignment (const char *string, int quoted)
{
  ...
  add_unwind_protect_heap (rboxc_dispose_expansion_word, rboxc_dispose_expansion_word, td);
  value = call_expand_word_internal (td, quoted, 0, (int *)NULL, (int *)NULL);
  discard_unwind_frame ("rboxc-assignment-expansion");
  dispose_word (td);
...
process_substitute (char *string, int open_for_read_in_child)
{
  ...
  add_unwind_protect (xfree, pathname);
...
#if defined (ARRAY_VARS)
static void
rboxc_discard_pipestatus_array (void *a)
{
  array_dispose (a);
}
...
/* Keep descriptor visibility through EXIT; the exit registry owns it. */
static void
...
function_substitute (char *string, int quoted, int flags)
{
  ...
  if (valsub == 0) rboxc_bash_track_backup (afd);
  ...
  if (parsing_command == 0)
    {
      ...
      add_unwind_protect_heap (uw_restore_pipestatus_array, rboxc_discard_pipestatus_array, psa);
    }
#endif
...
/*itrace("parameter_brace_expand_rhs: %s:%s pflags = %d", name, value, pflags);*/
  ...
  w = alloc_word_desc ();
  l_hasdollat = 0;
  begin_unwind_frame ("rboxc-brace-rhs");
  if (temp != value) add_unwind_protect (xfree, temp);
  add_unwind_protect_heap (rboxc_dispose_expansion_word, rboxc_dispose_expansion_word, w);
...
parameter_brace_expand_length (char *name)
{
  ...
  if (name[1] == '\0')			/* ${#} */
	  ...
	  if (newname && *newname)
	    {
	      ...
	      strcpy (newname + 1, nameref_cell (var));
              begin_unwind_frame ("rboxc-brace-length-reference");
      ...
      else				/* ${#PS1} */
	{
	  ...
	  newname[0] = '$';
          begin_unwind_frame ("rboxc-brace-length-expansion");
          add_unwind_protect (xfree, newname);
...
#  endif /* CASEMOD_TOGGLECASE */
  ...
  if (*name == '#' && name[1])
      ...
      if (string[sindex - 1] != RBRACE || (valid_length_expression (name) == 0))
	{
      ...
      begin_unwind_frame ("rboxc-brace-length-name");
...
bad_substitution:
      ...
      if (value == 0 || *value == '\0' || temp == 0 || *temp == '\0')
	{
      ...
      begin_unwind_frame ("rboxc-brace-pattern-call");
      add_unwind_protect (xfree, name);
      add_unwind_protect (xfree, temp);
      add_unwind_protect (xfree, value);
	  ...
	  if (c == '+')
		...
		*contains_dollar_at = 0;
	      ...
	      if (value)
		{
		  ...
		  if (quoted & (Q_HERE_DOCUMENT|Q_DOUBLE_QUOTES))
		    quoted |= Q_DOLBRACE;
		  begin_unwind_frame ("rboxc-brace-arguments");
          add_unwind_protect (xfree, name);
          add_unwind_protect (xfree, value);
	...
	*contains_dollar_at = 1;
      ...
      if (FUNSUB_CHAR (string[zindex + 1]))
	{
	  ...
	  /* we need zindex+1 because string[zindex] == RPAREN */
	    ...
	    {
	      begin_unwind_frame ("rboxc-function-substitution-source");
	      add_unwind_protect (xfree, temp);
...
arithsub:
	  ...
	  eflag = (shell_compatibility_level > 51) ? 0 : EXP_EXPANDED;
	  begin_unwind_frame ("rboxc-arithmetic-substitution");
	  add_unwind_protect (xfree, temp);
	  add_unwind_protect (xfree, temp1);
...
expand_word_internal (WORD_DESC *word, int quoted, int isexp, int *contains_dollar_at, int *expanded_something)
{
  ...
  struct rboxc_word_expansion *owned;
  ...
  owned = xmalloc (sizeof *owned);
  owned->text = (char *)xmalloc (istring_size = DEFAULT_INITIAL_ARRAY_SIZE);
  begin_unwind_frame ("rboxc-word-expansion");
  add_unwind_protect_heap (rboxc_release_word_expansion, rboxc_release_word_expansion, owned);

  owned->text[istring_index = 0] = '\0';
...
add_string:
	  if (temp)
	    {
	      owned->text = sub_append_string (temp, owned->text, &istring_index, &istring_size);
	      temp = (char *)0;
	    }
	...
	case '$':
	  ...
	  if (tword == &expand_wdesc_error || tword == &expand_wdesc_fatal)
	    {
	      free (string);
	      { free (owned->text); owned->text = 0; }
	      discard_unwind_frame ("rboxc-word-expansion");
	      free (owned);
	...
	case '\\':
	  ...
	  /* This is the fix for " $@\ " */
	    ...
	    {
	      RESIZE_MALLOCED_BUFFER (owned->text, istring_index, 2, istring_size,
				      DEFAULT_ARRAY_SIZE);
	      owned->text[istring_index++] = CTLESC;
	      owned->text[istring_index++] = '\\';
	      owned->text[istring_index] = '\0';
...
add_twochars:
	  ...
	  RESIZE_MALLOCED_BUFFER (owned->text, istring_index, 2, istring_size,
				  DEFAULT_ARRAY_SIZE);
	  owned->text[istring_index++] = twochars[0];
	  owned->text[istring_index++] = twochars[1];
	  owned->text[istring_index] = '\0';
	  ...
	  if (temp && *temp)
		...
		tword->flags |= W_ASSIGNRHS;
	      ...
	      /* XXX - preserve Q_ARITH here? */
              begin_unwind_frame ("rboxc-quoted-word");
              add_unwind_protect_heap (rboxc_dispose_expansion_word, rboxc_dispose_expansion_word, tword);
	...
	case '\'':
	  ...
	  /* If we have a quoted null expansion, add a quoted NULL to owned->text. */
	  if (temp == 0)
	    {
	      c = CTLNUL;
	      sindex--;		/* add_character: label increments sindex */
...
add_character:
	  RESIZE_MALLOCED_BUFFER (owned->text, istring_index, 1, istring_size,
				  DEFAULT_ARRAY_SIZE);
	  owned->text[istring_index++] = c;
	  owned->text[istring_index] = '\0';
...
finished_with_string:
  ...
  if (*owned->text == '\0')
...
#endif
	{
	  ...
	  owned->text[1] = '\0';
	  tword = alloc_word_desc ();
	  tword->word = owned->text;
	  owned->text = 0;		/* avoid later free() */
  ...
  else if (word->flags & W_NOSPLIT)
    {
      tword = alloc_word_desc ();
      tword->word = owned->text;
      if (had_quoted_null && QUOTED_NULL (owned->text))
    ...
    {
      list = list_string (owned->text, "", quoted);
      tword = list->word;
      if (had_quoted_null && QUOTED_NULL (owned->text))
	tword->flags |= W_HASQUOTEDNULL;
      free (list);
      ...
      if (split_on_spaces)
	{
	  ...
	  if (ifs_is_set == 0)
	    list = list_string (owned->text, " \t\n", 1);	/* XXX quoted == 1? */
	  else
	    list = list_string (owned->text, " ", 1);	/* XXX quoted == 1? */
	}
	  ...
	  if (*ifs_chars && *ifs_chars != ' ')
	    {
		 ...
		 quoting we added back in param_expand(). */
	      list = list_string (owned->text, *ifs_chars ? ifs_chars : " ", 1);
      ...
      else
	{
	  ...
	  if (expanded_something && *expanded_something == 0 && has_quoted_ifs)
	    tword->word = remove_quoted_ifs (owned->text);
	  else
	    tword->word = owned->text;
	  if (had_quoted_null && QUOTED_NULL (owned->text))	/* should check for more than one */
	    tword->flags |= W_HASQUOTEDNULL;	/* XXX */
...
struct rboxc_expanded_words { WORD_LIST *original, *expanded; };
...
shell_expand_word_list (WORD_LIST *tlist, int eflags)
{
  ...
  add_unwind_protect_heap (rboxc_dispose_expanded_words, rboxc_dispose_expanded_words, owned);
  ...
  for (orig_list = tlist; tlist; tlist = next)
      ...
      if (expanded == &expand_word_error || expanded == &expand_word_fatal)
	{
	  ...
	  run_unwind_frame ("rboxc-shell-expand");
  ...
  discard_unwind_frame ("rboxc-shell-expand");
  free (owned);
  if (orig_list)  
