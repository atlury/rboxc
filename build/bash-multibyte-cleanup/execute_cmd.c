...
execute_pipeline (COMMAND *command, int asynchronous, int pipe_in, int pipe_out, struct fd_bitmap *fds_to_close)
{
  ...
  while (cmd && cmd->type == cm_connection &&
	...
	cmd->value.Connection->first->flags |= CMD_IGNORE_RETURN;
      ...
      if (prev >= 0)
	rboxc_close_pipeline_fd (prev);

      prev = fildes[0];
      rboxc_close_pipeline_fd (fildes[1]);
...
execute_arith_command (ARITH_COM *arith_command)
{
  ...
  if (exp)
    {
      eflag = (shell_compatibility_level > 51) ? 0 : EXP_EXPANDED;
      begin_unwind_frame ("rboxc-arithmetic-command");
...
execute_function (SHELL_VAR *var, WORD_LIST *words, int flags, struct fd_bitmap *fds_to_close, int async, int subshell)
{
  ...
  if (subshell == 0)
    {
	 ...
	 OPTIND to force a getopts state reset. */
      add_unwind_protect_owned (uw_maybe_restore_getopt_state, xfree, gs);
  ...
  if (subshell == 0)
    add_unwind_protect_owned (uw_restore_funcarray_state, xfree, fa);
  else
  ...
  if (subshell) run_unwind_frame ("rboxc-function-heap");
  function_misc_cleanup ();
  return (result);
}
  ...
  */
  ...
  if (pid == 0)
	...
	interactive = 0;
      ...
      begin_unwind_frame ("rboxc-external-redirection-strings");
      ...
      if (redirects && (do_redirections (redirects, RX_ACTIVE) != 0))
	{
      ...
      discard_unwind_frame ("rboxc-external-redirection-strings");
	  ...
	  if (hookf == 0)
	    {
	      /* Make sure filenames are displayed using printable characters */
              char *rboxc_display_name = printable_filename (pathname, 0);
