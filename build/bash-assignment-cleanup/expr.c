...
/* Transfer a lookahead token back to the normal expression owner. */
...
expassign (void)
{
  ...
  if (curtok == EQ || curtok == OP_ASSIGN)
	...
	evalerror (_("arithmetic syntax error in variable assignment"));
      ...
      lhs = rboxc_expr_hold (savestring (tokstr));
      ...
      if (special)
	{
      ...
      rhs = rboxc_expr_hold (itos (value));
      if (noeval == 0)
	{
#if defined (ARRAY_VARS)
	  if (lind != -1)
...
exp0 (void)
{
  ...
  if (curtok == PREINC || curtok == PREDEC)
	...
	evalerror (_("identifier expected after pre-increment or pre-decrement"));
      ...
      vincdec = rboxc_expr_hold (itos (v2));
      if (noeval == 0)
	{
#if defined (ARRAY_VARS)
	  if (curlval.ind != -1)
