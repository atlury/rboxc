...
uwp_init (void)
{
  ocache_flush (uwcache, UNWIND_ELT);
...
/* Preserve the normal callback while specifying ownership on exec restart. */
void
...
/* The discard callback owns heap data and never references a C stack slot. */
void
...
/* Discard an abandoned error frame while releasing known heap payloads.
