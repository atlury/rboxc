...
/* All table nodes belong to one command conversion, including nodes whose
...
process_fd (struct convtable *tbl, int fd, FILE *output)
{
  ...
  char *inptr = rboxc_input_buffer;
  size_t actlen = 0;

  while (actlen < rboxc_input_capacity)
    {
      ssize_t n = read (fd, inptr, rboxc_input_capacity - actlen);
  ...
  if (actlen == rboxc_input_capacity)
    while (1)
      {
	ssize_t n;
	char *new_inbuf;
	...
	/* Increase the buffer.  */
	new_inbuf = (char *) realloc (rboxc_input_buffer, rboxc_input_capacity + 32768);
	if (new_inbuf == NULL)
	  {
	    error (0, errno, _("unable to allocate buffer for input"));
	    return -1;
	  }
	rboxc_input_buffer = new_inbuf;
	rboxc_input_capacity += 32768;
	inptr = rboxc_input_buffer + actlen;

	do
	  {
	    n = read (fd, inptr, rboxc_input_capacity - actlen);
  ...
  return process_block (tbl, rboxc_input_buffer, actlen, output);
}
