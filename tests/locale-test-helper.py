...
report_path=ROOT/'evidence/locale-test-helper-archive-contract.json';assert not report_path.exists()
 ...
 with tempfile.TemporaryDirectory(prefix='locale-contract-',dir=ROOT/'build') as directory:
  ...
  if fixture!='system':
   ...
   if fixture=='archive-directory-aliases':
    (loc/'locale-archive').touch();mounts += [str(archive),'/usr/lib/locale/locale-archive']
  for args in [['-a'],['-v','-a']]:
   ...
   if fixture=='archive-directory-aliases' and args==['-a']:assert b'en_US.utf8\n' in bytes.fromhex(reference['stdout'])
