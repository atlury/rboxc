#!/usr/bin/env python3
"""Compare copied child-signal trap ownership and control flow with GNU Bash."""
...
profile=ComparisonProfile('bash-sigchld-contract',selections=True)
...
cases=[]
scripts=[('repeat', "trap ':' CHLD; for ((i=0;i<12;i++)); do : & wait; done; trap - CHLD; printf 'complete\\n'"), ('replace', 'seen=; trap \'seen=yes; trap "seen=yes" CHLD\' CHLD; : & wait; trap - CHLD; printf \'seen:%s\\n\' "$seen"'), ('reset', 'seen=; trap \'seen=yes; trap - CHLD\' CHLD; : & wait; printf \'seen:%s\\n\' "$seen"'), ('function', 'f(){ seen=; trap \'seen=yes\' CHLD; : & wait; trap - CHLD; printf \'seen:%s\\n\' "$seen"; return 7; }; f; printf \'status:%s\\n\' "$?"'), ('queued', "trap ':' CHLD; : & for ((i=0;i<10000;i++)); do :; done; wait; trap - CHLD; printf 'queued\\n'")]
files={}
...
for index,(name,command,args,data,files) in enumerate(cases):
 ...
 report={**profile.metadata(),'inputs':inputs,'scope':'Five bounded child-signal lifetime cases through four shell aliases compare native GNU with the immutable Rboxc candidate. Includes queued and repeated dispatch, active replacement/reset and function status preservation. All child jobs belong to the test. Complete parent and child Valgrind summaries are required. Native findings remain baseline observations; this is focused ownership evidence, not full Bash acceptance.',
