#!/usr/bin/env python3
"""Check interactive terminal ownership with and without a controlling tty."""
...
profile=ComparisonProfile('bash-terminal-contract',selections=True,oracle=ROOT/'build/gnu-bash/bash')
...
cases=[]
script="trap 'test -e /proc/self/fd/255 && echo terminal-open-at-exit' EXIT; test -t 255; printf 'tty:%s\\n' \"$?\"; case $- in *m*) echo monitor:on;; *) echo monitor:off;; esac; exit 7"
for alias in ('bash','sh','-bash','-sh'):
 for terminal in (False,True):
  cases.append((alias+(':'+str(terminal)),alias,['-i','-c',script],terminal,{}))
if profile.options.commands:cases=[c for c in cases if c[0] in profile.options.commands]
results=[]
for index,(name,command,args,terminal,files) in enumerate(cases):
 outcomes={}
  ...
  for instrument in (False,True):
   ...
   with tempfile.TemporaryDirectory(prefix='rboxc-bash-') as directory:
    ...
    if terminal:
     ...
     def attach():
      os.setsid();fcntl.ioctl(slave,termios.TIOCSCTTY,0);os.close(slave)
     options={'preexec_fn':attach,'pass_fds':(slave,)}
    try:
     done=subprocess.run(argv,input=b'',cwd=work/'files',capture_output=True,timeout=60,**options,
       env={'LC_ALL':'C','LANGUAGE':'C','PATH':str(work/'exec')+':/usr/bin:/bin','HOME':directory,'TZ':'UTC0'})
    finally:
     if terminal:os.close(slave);os.close(master)
    (logs/'stdout').write_bytes(done.stdout);(logs/'stderr').write_bytes(done.stderr)
    diagnostics=done.stderr
    if not terminal:
     pattern=rb'cannot set terminal process group \(-?[0-9]+\): Inappropriate ioctl for device'
     assert len(re.findall(pattern,diagnostics))==1
     assert diagnostics.count(b'no job control in this shell')==1
     diagnostics=re.sub(pattern,b'cannot set terminal process group (<session>): Inappropriate ioctl for device',diagnostics)
    else:assert b'job control' not in diagnostics
    assert done.returncode==7 and b'terminal-open-at-exit\n' in done.stdout
    row={'status':done.returncode,'stdout':done.stdout.replace(directory.encode(),b'<fixture>').hex(),
         'stderr':diagnostics.replace(directory.encode(),b'<fixture>').hex(),'raw_stdout':done.stdout.hex(),'raw_stderr':done.stderr.hex(),
 ...
 results.append({'name':name,'command':command,'args':args,'controlling_terminal':terminal,'equivalent':equivalent,'memory_clean':clean,'pass':equivalent and clean,'outcomes':outcomes})
 for path,value in inputs.items():assert fingerprint(Path(path))==value
 report={**profile.metadata(),'inputs':inputs,'scope':'Eight interactive startup/EXIT contracts cover four aliases with and without a private controlling terminal. Terminal fd 255 stays available through EXIT, and original monitor/tty state and exit status match GNU. Only the independently validated missing-terminal session number is canonicalized for comparison; raw diagnostics remain recorded. Every candidate process must finish with clean Valgrind logs.',
