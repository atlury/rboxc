#!/usr/bin/env python3
"""Compare individually reviewed GNU Grep original scripts."""
...
profile = ComparisonProfile('grep-original', oracle=ROOT/'build/gnu-grep/src/grep', selections=True)
...
for index, row in enumerate(manifest['scripts']):
    ...
    for implementation in ('gnu', 'rboxc'):
        for instrument in (False, True):
            ...
            with tempfile.TemporaryDirectory(prefix='rboxc-grep-original-') as directory:
                work = Path(directory)
