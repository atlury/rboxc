#!/usr/bin/env python3
"""Compare bounded RETURN trap ownership and control flow with GNU Bash."""
...
profile=ComparisonProfile('bash-return-trap-contract',selections=True)
...
cases=[]
scripts=[('unchanged', "set -T; trap ':' RETURN; f(){ :; }; for ((i=0;i<25;i++)); do f; done; printf 'unchanged\n' "), ('debug-functions', "set -T; d(){ :; }; r(){ :; }; trap d DEBUG; trap r RETURN; f(){ :; :; }; for ((i=0;i<25;i++)); do f; done; trap - DEBUG RETURN; printf 'debug\n' "), ('replace-active', 'set -T; trap \'trap "echo next" RETURN; trap ":" RETURN; echo first\' RETURN; f(){ :; }; f; f; echo done'), ('reset-active', 'set -T; trap \'trap - RETURN; trap ":" RETURN; trap - RETURN; echo reset\' RETURN; f(){ :; }; f; f'), ('sourced-return', 'set -T; trap \':\' RETURN; d(){ :; }; trap d DEBUG; source ./input one two; printf \'source:%s\n\' "$?" '), ('return-status', 'set -T; trap \':\' RETURN; f(){ return 7; }; f; printf \'status:%s\n\' "$?" '), ('substitution', 'set -T; trap \':\' RETURN; d(){ :; }; trap d DEBUG; f(){ x=$(printf value); printf \'%s\n\' "$x"; }; f'), ('handler-restart', "set -T; trap 'exec ./plain return-handler' RETURN; f(){ :; }; f; printf unreachable")]
...
for index,(name,command,args,data,files) in enumerate(cases):
 ...
 report={**profile.metadata(),'inputs':inputs,'scope':'Eight bounded RETURN-trap lifetime cases through four shell aliases compare native GNU with the immutable Rboxc candidate. Includes active replacement/reset, DEBUG callbacks, sourced returns, status preservation, substitution and interpreter restart. Complete parent and child Valgrind summaries are required. Native findings remain baseline observations; this is focused ownership evidence, not full Bash acceptance.',
