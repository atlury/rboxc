...
for row in selected:
    ...
    for implementation,binary in [('gnu',profile.oracle),('rboxc',profile.binary)]:
        for instrument in (False,True):
            ...
            with tempfile.TemporaryDirectory(prefix='rboxc-bash-original-') as directory:
                ...
                for n,p in helpers.items():(work/'exec'/n).symlink_to(p)
    ...
    passed=all(o['expected_output_matches'] for o in outcomes.values()) and outcomes['rboxc-valgrind']['memory_clean']
    ...
    report={**profile.metadata(),'inputs':{str(p):h for p,h in inputs.items()},
        'scope':'Reviewed complete Bash scripts execute unchanged in private directories. Compare their original expected files using the output mode/filter from the original run recipe. Trace shell children and pinned GNU text helpers; preserve native GNU memory findings. Other original scripts remain open.',
