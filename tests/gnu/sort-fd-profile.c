/* Restore the sort test's effective seven-descriptor limit before main; preserve Valgrind's hard ceiling. */
...
__attribute__((constructor)) static void restore_sort_limit(void)
{
  ...
  struct rlimit limit, actual;
  if (getrlimit(RLIMIT_NOFILE, &limit)) _exit(125);
  rlim_t tool_hard_limit = limit.rlim_max;
  limit.rlim_cur = 7;
  if (setrlimit(RLIMIT_NOFILE, &limit) || getrlimit(RLIMIT_NOFILE, &actual)) {
    dprintf(2, "sort fd profile: set/get limit failed: %d\n", errno); _exit(125);
  }
  if (actual.rlim_cur != 7 || actual.rlim_max != tool_hard_limit) {
  ...
  int len = snprintf(record, sizeof record, "{\"pid\":%ld,\"soft\":7,\"hard\":%lu,\"before_main\":true}\n", (long)getpid(), (unsigned long)actual.rlim_max);
