#!/usr/bin/env python3
"""Compare ordinary pending-signal returns, restarts and handler replacement."""
...
profile=ComparisonProfile('bash-trap-state-contract',selections=True)
...
cases=[]
scripts=[('user-signal-normal', 'trap \'printf signal\\n\' USR1; kill -s USR1 $$; printf \'after:%s\\n\' "$?"'), ('user-signal-restart', "trap 'exec ./plain user-signal' USR1; kill -s USR1 $$; printf unreachable"), ('user-signal-return', 'trap \'return 7\' USR1; f(){ kill -s USR1 $$; printf unreachable; }; f; printf \'returned:%s\\n\' "$?"'), ('interrupt-return', 'trap \'return 9\' INT; f(){ kill -s INT $$; printf unreachable; }; f; printf \'returned:%s\\n\' "$?"'), ('debug-reset-restart', "trap 'trap - DEBUG; exec ./plain reset-debug' DEBUG; :"), ('error-reset-restart', "trap 'trap - ERR; exec ./plain reset-error' ERR; false"), ('return-reset-restart', "set -T; trap 'trap - RETURN; exec ./plain reset-return' RETURN; f(){ :; }; f"), ('interrupt-reset-restart', "trap 'trap - INT; exec ./plain reset-interrupt' INT; kill -s INT $$")]
...
for index,(name,command,args,data,files) in enumerate(cases):
 ...
 report={**profile.metadata(),'inputs':inputs,'scope':'Eight bounded cases through four shell aliases compare native GNU and Rboxc: ordinary self-directed USR1, restart from USR1, return from USR1/INT within a function, and DEBUG/ERR/RETURN/INT handlers that reset themselves before executing a private plain script. All candidate process summaries must be complete and clean; native findings are preserved.',
