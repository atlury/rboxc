...
cases=[]
scripts=[('function-scope', 'v=outer; f(){ local v=inner; trap \'printf "scope:%s:%s:%s\\n" "$v" "${FUNCNAME[*]}" "$BASH_COMMAND" >&2\' EXIT; printf \'%s\' "${ exit 7; }"; }; f'), ('getopt-scope', 'OPTIND=1; set -- -a; getopts a opt; f(){ local OPTIND=4; trap \'printf "options:%s:%s\\n" "$OPTIND" "$opt" >&2\' EXIT; printf \'%s\' "${ local OPTIND=8; exit 9; }"; }; f'), ('nested-scope', 'v=outer; trap \'printf "nested:%s:%s\\n" "$v" "$?" >&2\' EXIT; printf \'%s\' "${ local v=middle; printf \'%s\' "${ local v=inner; exit 11; }"; }"')]
files={}
