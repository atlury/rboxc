...
cases += [
    ('closed-stdin-help', ['--help']), ('closed-stdin-version', ['--version']),
    ('closed-stdin-list', ['-tf', 'archive.tar']),
    ('closed-stdin-missing-archive', ['-tf', 'absent.tar']),
    ('closed-stdin-read', ['-tf', '-']),
    ('closed-stdout-version', ['--version']),
    ('closed-stderr-version', ['--version']),
...
for index, (name, arguments) in enumerate(cases):
    ...
    for implementation in ('gnu', 'rboxc'):
        for instrument in (False, True):
            ...
            with tempfile.TemporaryDirectory(prefix='rboxc-tar-behavior-') as directory:
                ...
                closed_fd = closed_streams[name.split('-')[1]] if name.startswith('closed-') else None
                child_setup = (lambda: os.close(closed_fd)) if closed_fd is not None else None
                with open('/dev/full' if name.endswith('-full') else os.devnull, 'wb') as sink:
                    done = subprocess.run(invocation, cwd=work, env=environment, stdin=subprocess.DEVNULL, stdout=sink if name.endswith('-full') else subprocess.PIPE, stderr=subprocess.PIPE, preexec_fn=child_setup, timeout=60)
                tree = {}
                links = {}
    ...
    results.append({'name': name, 'command': 'tar', 'arguments': arguments, 'environment': case_environment.get(name, {}), 'closed_standard_descriptor': closed_fd, 'pass': equivalent and clean, 'equivalent': equivalent, 'memory_clean': clean, 'outcomes': outcomes})
