...
profile=ComparisonProfile('findutils-special-original',oracle=ROOT/'build/gnu-findutils/find/find',selections=True)
source=Path(json.loads((ROOT/'inventory/sources.json').read_text())['findutils']['source'])
selections={'find/access':(4,[]),'find/fprint-unwritable':(4,[]),
            'find/execdir-in-unreadable':(4,[]),'find/execdir-multiple':(8,[]),
 ...
 for implementation in ('gnu','rboxc'):
  for instrument in (False,True):
   ...
   with tempfile.TemporaryDirectory(prefix='rboxc-findutils-original-') as directory:
    ...
    done=subprocess.run(invocation,cwd=cwd,env=env,user=65534,group=65534,extra_groups=[],stdout=subprocess.PIPE,stderr=subprocess.STDOUT,timeout=1800 if name=='exec-nogaps' else 180)
