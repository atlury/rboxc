#!/usr/bin/env python3
"""Compare no-fork substitution state and cleanup through return and exit."""
...
profile=ComparisonProfile('bash-function-substitution-contract',selections=True)
...
cases=[]
scripts=[('normal', 'v=outer; x=${ v=inner; printf output; }; printf "%s:%s\\n" "$v" "$x"'), ('return', 'x=${ printf output; return 7; printf unreachable; }; printf "%s:%s\\n" "$?" "$x"'), ('subshell-exit', '( printf "%s\\n" "${ printf hidden; exit 7; }" ); printf "outer:%s\\n" "$?"'), ('nested', 'printf "%s\\n" "${ printf one; printf %s "${ printf two; }"; }"'), ('value-local', 'REPLY=outer; printf "%s:%s\\n" "${| REPLY=inner; }" "$REPLY"'), ('stdout-closed', 'exec 1>&-; x=${ printf output; }; printf "%s\\n" "$x" >&2'), ('errexit', 'set -e; x=${ set -e; printf hidden; false; }; printf unreachable'), ('exit-trap', 'trap \'printf exit >&2\' EXIT; printf \'%s\\n\' "${ printf hidden; exit 7; }"')]
files={}
...
for index,(name,command,args,data,files) in enumerate(cases):
 ...
 report={**profile.metadata(),'inputs':inputs,'scope':'Eight bounded no-fork substitution contracts through four shell aliases compare GNU output, status and private file effects. Normal output capture, local REPLY, nested substitutions, return, subshell exit, closed stdout, errexit and EXIT traps exercise temporary descriptor and source-string lifetime. All child logs are retained; native GNU findings remain separate',
