#!/usr/bin/env python3
"""Compare arithmetic caller ownership through normal values and index diagnostics."""
...
profile=ComparisonProfile('bash-arithmetic-contract',selections=True)
...
cases=[]
scripts=[('normal-index', 'a=([2]=four); printf "%s\\n" "$((a[1+1]+1))"; ((a[2]++)); declare -p a'), ('assignment-index', 'declare -a a; key="~-2"; a[$key]=42; printf "after:%s\\n" "$?"'), ('command-index', 'declare -a a; key="x+"; ((a[$key]++)); printf "after:%s\\n" "$?"'), ('substitution-index', 'declare -a a; key="x+"; printf "%s\\n" "$((a[$key]))"; printf "after:%s\\n" "$?"'), ('legacy-index', 'declare -a a; key="x+"; printf "%s\\n" "$[a[$key]]"; printf "after:%s\\n" "$?"'), ('function-index', 'f(){ local -a a; local key="x+"; ((a[$key]++)); printf unreachable; }; f; printf "outer:%s\\n" "$?"'), ('exit-trap-index', 'trap \'printf exit\' EXIT; declare -a a; key=\'x+\'; printf \'%s\\n\' "$((a[$key]))"'), ('associative-normal', 'declare -A a; key="a b"; a[$key]=3; ((a[$key]++)); printf "%s\\n" "${a[$key]}"')]
files={}
...
for index,(name,command,args,data,files) in enumerate(cases):
 ...
 report={**profile.metadata(),'inputs':inputs,'scope':'Eight bounded arithmetic and array-index contracts through four shell aliases compare original GNU output, status and private file effects. Ordinary diagnostics exercise caller ownership through arithmetic commands, substitutions, assignment, functions and EXIT traps. Native GNU memory findings remain separate',
