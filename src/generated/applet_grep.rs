...
unsafe extern "C" fn fillbuf(mut save: idx_t, mut st: *const stat) -> bool {
    ...
    if min_after_buflim <= buffer.offset(bufalloc as isize).offset_from(buflim) {
        ...
        if incr_min <= 0 as ptrdiff_t {
            ...
            libc::memset(newbuf.cast(), 0, bufalloc as usize);
        }
        readbuf = if (newbuf
            .offset(1 as ::core::ffi::c_int as isize)
...
pub unsafe extern "C" fn single_binary_main_grep(
            ...
            match opt {
                ...
                102 => {
                    ...
                    if ferror_unlocked(fp) == 0 {
                        ...
                        } else if rboxc_grep_close_patterns(fp) != 0 as ::core::ffi::c_int {
    ...
    buffer = rboxc_grep_allocate_buffer(bufalloc) as *mut ::core::ffi::c_char;
...
extern "C" {
    #[link_name = "rboxc_grep_GEAfree_all"]
    fn GEAfree_all();
    #[link_name = "rboxc_grep_Pfree_all"]
    fn Pfree_all();
