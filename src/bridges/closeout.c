...
/* stdout and stderr are static FILE objects on this glibc profile.
...
close_stdout (void)
{
  if (rboxc_close_standard_stream (stdout) != 0
  ...
  if (!SANITIZE_ADDRESS && rboxc_close_standard_stream (stderr) != 0)
    _exit (exit_failure);
}
