...
for row in selected:
    ...
    if row.get('absolute_helpers') or row.get('empty_system_profile'):
            ...
            with tempfile.TemporaryDirectory(prefix='rboxc-bash-original-') as directory:
                ...
                if row.get('absolute_helpers') or row.get('empty_system_profile'):
                    ...
                    for absolute in row.get('absolute_helpers',[]):
                        assert absolute in ('/bin/echo','/bin/sh','/bin/sed','/bin/ls','/bin/true','/bin/false','/bin/cat','/bin/mkdir','/bin/touch','/bin/chmod','/bin/rm','/usr/bin/true','/usr/bin/false')
