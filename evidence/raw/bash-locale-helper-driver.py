...
for row in selected:
    ...
    if row.get('locale_archive'):
        ...
        if row.get('locale_helper'):
            for field in ('binary','build','audit'):
                helper_path=ROOT/row['locale_helper'][field]
                inputs[helper_path]=row['locale_helper'][field+'_sha256']
    ...
    for implementation,binary in [('gnu',profile.oracle),('rboxc',profile.binary)]:
        for instrument in (False,True):
            ...
            with tempfile.TemporaryDirectory(prefix='rboxc-bash-original-',dir='/var/tmp' if row.get('unprivileged') else ROOT/'build' if row.get('private_tmp') else None) as directory:
                ...
                if row.get('locale_archive'):(work/'exec/locale').symlink_to(ROOT/row['locale_helper']['binary'] if row.get('locale_helper') else Path('/usr/bin/locale'))
                ...
                if row.get('absolute_helpers') or row.get('empty_system_profile') or row.get('locale_archive') or row.get('private_tmp'):
                    ...
                    for absolute in row.get('absolute_helpers',[]):
                        assert absolute in ('/bin/ln','/bin/cp','/bin/rmdir','/bin/env','/bin/sleep','/bin/echo','/bin/sh','/bin/sed','/bin/ls','/bin/true','/bin/false','/bin/cat','/bin/mkdir','/bin/touch','/bin/chmod','/bin/rm','/usr/bin/true','/usr/bin/false','/usr/bin/printf')
                    ...
                    if row.get('locale_archive'):
                        ...
                        private_locale={'destination':str(destination),'source':str(archive),'source_sha256':fingerprint(archive),
                                        'native_locale_helper':str(ROOT/row['locale_helper']['binary']) if row.get('locale_helper') else '/usr/bin/locale'}
