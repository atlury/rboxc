...
__attribute__((constructor)) static void restore_sort_limit(void)
{
  ...
  if (setrlimit(RLIMIT_NOFILE, &limit) || getrlimit(RLIMIT_NOFILE, &actual)) _exit(125);
  if (actual.rlim_cur != 7 || actual.rlim_max != 7) _exit(125);
  ...
  if (fd < 0 || write(fd, record, (size_t)len) != len || close(fd)) _exit(125);
