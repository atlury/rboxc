...
def main():
    ...
    sort_fd_library = None
    if instrument and any(row.get('sort_fd_profile') and included(row) for row in manifest):
        sort_fd_library = Path(launcher_workspace.name)/'sort-fd-profile.so'
        subprocess.run(['cc', '-shared', '-fPIC', '-O2', '-Wall', '-Wextra', '-Werror',
                        ROOT/'tests/gnu/sort-fd-profile.c', '-o', sort_fd_library], check=True)
    ...
    for row in manifest:
        ...
        if row.get('profile') == 'private-root':
            assert row['script'] in ('tests/chown/preserve-root.sh', 'tests/rm/r-root.sh')
            if row['script'] == 'tests/chown/preserve-root.sh':
                assert row['commands'] == ['chown', 'chgrp', 'chmod'] and row.get('native_launcher')
            else:
                assert row['commands'] == ['rm'] and row['full_suite'] and not instrument
        ...
        if row.get('valgrind_multicall'):
            ...
            assert row['script'] in ('tests/chroot/chroot-fail.sh', 'tests/cut/cut.pl', 'tests/sort/sort.pl'), 'multicall tracing requires a reviewed profile'
        ...
        if instrument and row.get('sort_fd_profile'):
            ...
            context['sort_fd_profile'] = {'library_sha256': fingerprint(sort_fd_library),
                'source_sha256': fingerprint(ROOT/'tests/gnu/sort-fd-profile.c'),
                'bash_sha256': fingerprint(ROOT/'tests/gnu/sort-fd-profile.bash')}
        ...
        if row.get('dd_marker_profile'):
            ...
            stub = (SOURCE/row['script']).read_text().split("cat > k.c <<'EOF' || framework_failure_\n", 1)[1].split('\nEOF\n', 1)[0]+'\n'
            context['dd_marker_profile'] = {'driver_sha256': fingerprint(ROOT/'tests/gnu/dd-marker-compiler.py'),
                'compiler': str(Path('/usr/bin/cc').resolve(strict=True)),
                'compiler_sha256': fingerprint(Path('/usr/bin/cc')),
                'source_sha256': hashlib.sha256(stub.encode()).hexdigest()}
        ...
        for implementation in ('gnu', 'rboxc'):
            ...
            with tempfile.TemporaryDirectory(prefix='rboxc-upstream-') as temporary:
                ...
                if row.get('shell_selection'):
                    # Selected source is under the private checkout. Stage the
                    # exact pinned bytes where an ordinary-user child can read
                    # them without exposing or chmodding the checkout.
                    (run/'selected').mkdir(mode=0o755)
                    execution_script = run/'selected'/script.name
                    shutil.copyfile(script, execution_script)
                    execution_script.chmod(0o444)
                    assert fingerprint(execution_script) == row['shell_selection']['selected_sha256']
                    ...
                    if instrument:
                        ...
                        if instrument and row.get('sort_fd_profile'):
                            ...
                            startup += ('if [ "${RBOXC_SORT_FD_LIMIT-}" = 7 ]; then\n'
                                ...
                                '  LD_PRELOAD='+shlex.quote(str(library))+':${LD_PRELOAD-}\n'
                                '  export RBOXC_SORT_FD_LIMIT RBOXC_SORT_FD_JOURNAL LD_PRELOAD\nfi\n')
                        framework_env = ''
                ...
                if row.get('dd_marker_profile'):
                    ...
                    wrapper = run/'src/dd-marker-cc'
                    wrapper.write_text('#!/bin/sh\nexec '+shlex.join([sys.executable,
                        str(ROOT/'tests/gnu/dd-marker-compiler.py'), str(config)])+' "$@"\n')
                    wrapper.chmod(0o755)
                    environment['CC'] = str(wrapper)
                ...
                if script.suffix == '.pl':
                    ...
                    command = [test_shell, '-c', 'exec "$1" "$2" 9>&2', 'test', test_shell, str(execution_script)]
                nss_profile = None
                ...
                if row.get('dd_marker_profile'):
                    ...
                    record = json.loads(journal.read_text()) if journal.exists() else None
                    evidence = ROOT/'evidence/raw'/f'reviewed-dd-marker-{run.name}-{implementation}.json'
                    evidence.write_text(json.dumps(record, indent=2)+'\n')
                    outcomes[implementation]['dd_marker_profile'] = {'record': record,
                        'log': str(evidence.relative_to(ROOT)), 'sha256': fingerprint(evidence)}
                if memory_limit is not None:
                ...
                if row.get('profile') == 'private-root':
                    ...
                    if row['script'] == 'tests/rm/r-root.sh':
                        ...
                        expected_calls = ['exercise_rm_r_root '+arg for arg in (
                            ...
                            '--preserve-root file1 / file2', '--preserve-root //.',
                            '--preserve-root /./', '--preserve-root /.//', '--preserve-root /../',
                            '--preserve-root /.././', '--preserve-root /etc/..',
                            '--preserve-root rootlink/..', '--preserve-root rootlink2/.',
