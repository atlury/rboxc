...
target = ROOT/'evidence/tar-child-input-contract.json'
assert not target.exists()
stage = ROOT/'build/tar-child-input-contract'
logs = ROOT/'evidence/raw/tar-child-input-contract'
stage.mkdir()
logs.mkdir()
...
results = []
processes = []
for name in ['owned', 'inherited', 'reused', 'replaced', 'failed-stat']:
    outcomes = {}
...
target.write_text(json.dumps({
    'scope': 'Five native helper lifetime contracts using the exact adapted ownership and xdup2 source. Inherited child input, descriptor reuse, replacement and failed ownership capture are checked independently of archive assertions. GNU fatal-diagnostic plumbing is stubbed in this isolated helper probe; production integration is separate.',
    'passed': 5, 'total': 5, 'clean_processes': len(processes),
...
print('Five input ownership contracts pass; six process logs are clean')
