#!/usr/bin/env python3
"""Close the quota test stub's policy input after its scheduler lookup."""
...
if 'k.c' in args:
    ...
    before=b'  if (pid == 0 && fscanfmt (policyf, "policy : %d", &policy) == 1)'
    after=b'  int matched = pid == 0 && fscanfmt (policyf, "policy : %d", &policy) == 1;\n  fclose (policyf);\n  if (matched)'
    assert original.count(before)==1
    changed=original.replace(before,after)
    output=Path('k.policy-closed.c');assert not output.exists()
