...
for instrument in [False,True]:
 ...
 for impl in ['gnu','rboxc']:
  with tempfile.TemporaryDirectory(prefix='rboxc-inetd-') as directory:
   ...
   shutil.copy2(binary,work/'inetd-real')
   (work/'inetd').symlink_to('inetd-real')
   memory=work/'memory';memory.mkdir();os.chown(memory,65534,65534)
   if instrument:
    (work/'inetd').unlink()
    (work/'inetd').write_text('#!/bin/sh\nexec '+shlex.join(['/usr/bin/valgrind','--leak-check=full','--show-leak-kinds=all','--track-fds=yes','--trace-children=yes','--log-file='+str(memory/'%p.log'),str(work/'inetd-real')])+' "$@"\n')
