...
selections.update({
    'sparse05': (173, 'sparse05.at'),
    'star-gtarfail': (239, 'star/gtarfail.at'),
    'star-gtarfail2': (240, 'star/gtarfail2.at'),
    'star-multi-fail': (241, 'star/multi-fail.at'),
    'star-ustar-big-2g': (242, 'star/ustar-big-2g.at'),
    'star-ustar-big-8g': (243, 'star/ustar-big-8g.at'),
    'star-pax-big-10g': (244, 'star/pax-big-10g.at'),
    'multiv07': (163, 'multiv07.at'),
    'exclude17-generated': (76, 'testsuite'),
    'exclude18-generated': (77, 'testsuite'),
})
selections['extrac11'] = (94, 'extrac11.at')
generated_selections = {'exclude17-generated', 'exclude18-generated'}
external_selections = {'star-multi-fail', 'star-ustar-big-8g', 'star-gtarfail', 'multiv07', 'star-ustar-big-2g', 'star-gtarfail2', 'star-pax-big-10g'}
# Legacy fixture account names resolve only against this test's private files database.
nss_selections.update(external_selections)
large_selections = {'sparse05', 'star-ustar-big-2g', 'star-ustar-big-8g', 'star-pax-big-10g'}
compression_selections.update({name: 'bzip2' for name in large_selections if name != 'sparse05'})
selected = profile.options.commands or list(selections)
assert set(selected) <= set(selections)
helpers = {n: ROOT/'build/gnu-coreutils/src/coreutils' for n in ('cat','rm','mkdir','chmod','touch','sort','echo','basename','cp','ln','true','false','sleep','ls','mv','mktemp','cut','id','date','printf','dd','rmdir','expr','tr','wc','head','tail','uname','cksum','chown','md5sum')}
...
if set(selected) & external_selections:
    ...
    rows = json.loads(fixture_manifest.read_text())['fixtures']
    for name in set(selected) & external_selections:
        external_fixtures[name] = [row for row in rows if row['selection_source'] == str(source/'tests'/selections[name][1])]
        assert external_fixtures[name]
        for row in external_fixtures[name]:
            path = ROOT/row['path']
            assert fingerprint(path) == row['sha256']
            inputs[path] = fingerprint(path)
if set(selected) & large_selections:
    assert shutil.disk_usage('/tmp').free >= 16 * 1024**3
    assert shutil.disk_usage(ROOT).free >= 60 * 1024**3
for name in selected:
    path = source/'tests'/selections[name][1]
    if name in generated_selections:
        reviewed = manifest['generated_groups'][str(selections[name][0])]
        assert reviewed['reviewed'] and fingerprint(path) == reviewed['generated_suite_sha256']
        group = path.read_text().split('#AT_START_'+str(selections[name][0])+'\n', 1)[1].split('#AT_STOP_'+str(selections[name][0])+'\n', 1)[0]
        import hashlib
        assert hashlib.sha256(group.encode()).hexdigest() == reviewed['group_sha256']
        inputs[path] = fingerprint(path)
        continue
        ...
        for instrument in (False, True):
            ...
            with tempfile.TemporaryDirectory(prefix='rboxc-tar-original-') as directory:
                ...
                for sub in ('exec', 'real', 'deps', 'memory', 'copies'):
                    (work/sub).mkdir()
                copied_fixtures = []
                if name in external_fixtures:
                    (work/'download').mkdir()
                    for fixture in external_fixtures[name]:
                        original = ROOT/fixture['path']
                        copied = work/'download'/original.name
                        shutil.copy2(original, copied)
                        assert fingerprint(copied) == fixture['sha256']
                        copied_fixtures.append({'source': str(original), 'private_path': str(copied.relative_to(work)), 'sha256': fixture['sha256']})
                copies = {}
                ...
                timed_out = False
                try:
                    output, _ = process.communicate(timeout=1800 if name in large_selections else 300)
                ...
                assert all(fingerprint(work/f['private_path']) == f['sha256'] for f in copied_fixtures)
                ...
                outcomes[key] = {'status': done.returncode, 'assertions': assertions, 'assertions_pass': passed,
                                 ...
                                 'timeout_seconds': 1800 if name in large_selections else 300,
                                 'copied_test_fixtures': copied_fixtures,
