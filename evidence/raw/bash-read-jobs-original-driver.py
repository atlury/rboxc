            ...
            with tempfile.TemporaryDirectory(prefix='rboxc-bash-original-') as directory:
                ...
                if row.get('absolute_helpers'):
                    ...
                    for absolute in row['absolute_helpers']:
                        assert absolute in ('/bin/echo','/bin/sh','/bin/true','/bin/cat','/bin/mkdir','/bin/touch','/bin/chmod','/bin/rm','/usr/bin/true','/usr/bin/false')
                        destination=Path(absolute).resolve()
                        assert str(destination) in row['host_inputs']
                        native=profile.oracle if absolute=='/bin/sh' else ROOT/'build/gnu-coreutils/src/coreutils'
