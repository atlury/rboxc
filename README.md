...
required destructors; a heap-owned word descriptor survives nonlocal expansion
errors. Both comsub-eof and parser pass strictly with unchanged output.
[evidence/bash-parser-cleanup-validation.json](evidence/bash-parser-cleanup-validation.json)
retains that complete 41-script regression. The earlier trace cleanup and seven
...
`evidence/gawk-input-accounting-coverage.json` audits **542 passing original recipes,
six unchanged GNU baseline failures and 731 clean processes**: 570 original
Gawk invocations, forty-four shell children, thirty-two native helper children and
...
capability guards. All 634 distributed AWK inputs are now accounted, with none pending review. Three distributed utilities pass separate comparisons, adding no original-assertion count. Twelve further reviewed originals pass their
...
The two fork originals and bounded date-comparison original now pass, adding 29 clean candidate/helper processes. Fork filenames are bound to the parent-child PID relation in raw logs; date retries are limited by the original ten-attempt loop. A private native date helper releases its owned timezone and optional format copy at normal exit, matching the translated date cleanup. Only its date archive member changes. The original helper’s 128-byte-per-process findings remain preserved; candidate bytes are unchanged.
...
A supplemental timeout run passes the unchanged expected output in all four profiles when shell and sleep children run natively. Its one clean candidate Gawk log is audited separately; native child memory is outside that profile, and the full-child timing limitation remains open.
...
glibc sources; the GNU message catalog remains unchanged. The directory recipe
compares native `fts` output with an AWK walk over two private directories and four
empty files. All fourteen Gawk, shell and native-helper processes are clean.

Seven more unchanged originals pass for duplicate includes, a cross-reference
pretty-print round trip, numeric strings, assignment evaluation, source-byte
diagnostics and output to 1,030 private files. The source program processed by
the cross-reference test is recorded as text input, with no execution claim.
The new selections add fifteen clean candidate/helper processes; the candidate
and installed release are unchanged.
...
regex values. The byte sweep covers 256 values; all loops have fixed bounds.
All thirteen candidate process logs are clean, with no production binary change.

Five further originals pass for array parameter propagation, numeric-key formatting,
unassigned element types, Greek case matching and glossary processing. Two additional
array originals expose the same output differences in native GNU and Rboxc; their
unchanged expected output is retained, and they count as baseline failures. Two pipe
and exit-status recipes pass assertions with clean Gawk logs but native shell, sed
and cat findings. All 32 processes from those two profiles are recorded separately
from strict clean coverage. The initial child-command classification failure is
preserved alongside the completed run.

Nine debugger originals now pass for arrays, expression evaluation and typed
...
contains 572 original Gawk execution images. Heap and descriptor summaries describe
...
Five pass strictly. The runner waits for private children before preserving logs;
all delays and process groups are bounded. A separate unchanged GNU cat entry
built with `-Dlint` enables its existing normal-exit buffer cleanup and resolves
the child-input original. `gawk-cat-helper-validation.json` preserves the earlier
262,144-byte buffer leak and verifies three clean final processes. The six open
profiles retain native shell descriptor restoration, cached cat splice-pipe
descriptors, or the virtual-I/O extension descriptor probe. No Rboxc production
code or binary changes are part of this checkpoint.
...
library is accounted as an auxiliary source without adding a process or recipe.
Multi-output recipe failures are now preserved alongside conventional outputs.
